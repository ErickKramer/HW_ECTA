function xmin = CMA_ES_EP  % (mu/mu_w, lambda)-CMA-ES
    clear all; clc;
  % --------------------  Initialization --------------------------------  
  % Wing Parameters
   wing.numEvalPts = 256;
  ...
  stopeval = 20000;   % stop after stopeval number of function evaluations
...
%   eigeneval = 0;                      % track update of B and D
  ...
  while counteval < stopeval
      ...
      for k=1:lambda  
          arz(:,k) = randn(N,1);
          arx(:,k) = xmean + sigma * B * (D .* arz(:,k) ); % m + sig * Normal(0,C) 
          %x(:,k) = normalize(arx(:,k))
          foil = pts2ind(arx(:,k), wing.numEvalPts);
          arfitness(k) = feval(strfitnessfct, wing, foil); % objective function call
...
%          eigeneval = counteval;
         ...
         fitness(iFit) = feval(strfitnessfct, wing, foil);
