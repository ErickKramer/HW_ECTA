function output = TSP_German_ver(task, cityData, rate,  p )
    if nargin < 1
        %Load dataset
...
%         p.mutProb   = rate;
    ...
    for iGen = 1:p.maxGen
       ...
       if iGen == 1
            %function to create a matrix of random permutations for all the
            %individuals
       ...
       %Plot current best individual, just to animate the plot
%        subplot(1,2,1)
