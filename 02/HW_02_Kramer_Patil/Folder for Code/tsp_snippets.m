...
%% Fitness loop
fitness = distance_calc(pop, cityData.distMat);

%Plot of the best individual 
[lowest, ilowest] = min(fitness);
smallest = pop(ilowest,:);
%Plot from all the cities
plot(cityData.coords(1,smallest), cityData.coords(2,smallest),'-o');
hold on
%Plot from the last city to the first
plot([cityData.coords(1,smallest(end)), cityData.coords(1,smallest(1)) ], ...
    [cityData.coords(2,smallest(end)), cityData.coords(2,smallest(1)) ])
