...
wActive([1:nInputs], [nInputs+1:nInputs+nHidden]) = 1;
wActive([nInputs+1:nInputs+nHidden],[nInputs+nHidden+1:nInputs+nHidden+nOutputs] ) = 1;
...
%% Weights the weights in a vector and assign the new weights back

[wRow, wCol, weights] = find(wMat);
weights = weights';
weights = weights /2;
for i=1:length(wRow)
    wMat(wRow(i),wCol(i)) = weights(i);
end
